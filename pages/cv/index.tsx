...
import { FaMapMarkerAlt, FaPhoneAlt, FaLinkedin, FaLink, FaGithub, FaLuggageCart, FaCamera, FaGraduationCap, FaBriefcase, FaStar } from "react-icons/fa";
    ...
    const LeftEle = (obj: any) => {
        return (
            <div className="flex-initial md:w-72 h-fit md:sticky top-0 bottom-0 flex flex-col px-4 justify-center bg-lime-200">
                ...
                <h3 className="text-2xl py-2 text-center font-secondary font-semibold tracking-wider">{obj.name}</h3>
                ...
                <div className="pt-2 pb-4">
                            ...
                            return (
                                <li className="mb-6 ml-6 relative" key={item.title}>
                                    <span className="flex absolute -left-8 top-2 justify-center items-center w-4 h-4 bg-blue-200 rounded-full ring-8 ring-white">
                                        <svg className="w-2 h-2 text-blue-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd"></path></svg>
                                    </span>
                                    ...
                                    <div className="text-base font-normal text-gray-500">{item.institute}</div>
                                    <p className="mb-4 text-base font-normal text-gray-500">{item.text}</p>
                                </li>
                            )
                        })}
                    </ol>
                    ...
                    <ol className="border-l border-gray-200 ml-8 mt-3 pr-10">
                        {obj.experiences.map((item: any) => {
                            return (
                                <li className="mb-6 ml-6 relative text-gray-600" key={item.title}>
                                    <span className="flex absolute -left-8 top-2 justify-center items-center w-4 h-4 bg-blue-200 rounded-full ring-8 ring-white">
                                        <svg className="w-2 h-2 text-blue-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd"></path></svg>
                                    </span>
                                    <h3 className="flex items-center text-lg font-semibold text-gray-900">{item.title}</h3>
                                    <div className="pb-1"><a className="uppercase font-secondary font-light tracking-wider" href={item.link} target="_blank" rel="noreferrer">{item.company}, </a>{item.location}</div>
                                    <time className="block mb-2 text-sm font-normal leading-none text-gray-400">{item.time}</time>
                                    <div className="text-base font-normal">{item.institute}</div>
                                    <p className="mb-4 text-sm font-normal">{item.descriptions}</p>
                                    <p className="text-sm pb-1">{item.responsibilities.opening}</p>
                                    <ul className="list-disc list-inside">
                                        {item.responsibilities.list.map((res: string, index: number) => {
                                            return (
                                                <li className="text-sm pb-1" key={`res-${index}`}>{res}</li>
                                            )
                                        })}
                                    </ul>
                                </li>
                        ...
                        {obj.skills.map((item: any) => {
                            return (
                                <li className="mb-4 ml-6 relative" key={item.title}>
                                    <h3 className="flex items-center mb-1 text-lg text-primary">{item.title}</h3>
                                    <ul className="list-disc list-inside">
